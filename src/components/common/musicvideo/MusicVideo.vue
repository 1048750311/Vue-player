<template>
  <div class="musicvideo" ref="scroll">
    <div class="musicvideo-main">
      ...
      <div class="title">
        <span>热门标签:</span>
        <span v-for="(item, index) in catlist" :key="index" style="cursor: pointer;"
          :class="[curIndex === index ? 'actived' : '']" @click="changeMusicListCat(item.name, index)">
          {{ item.name }}
          <i style="paddingLeft:8px;color:#666;">{{ index === catlist.length - 1 ? '' : "|" }}</i>
        </span>
      </div>
      <div class="videosList">
        <VideoItem v-for="item in videosList" :key="item.id" :item="item.data" :wth="24" :topShow="false"
          :userShow="true" v-show="isTog"></VideoItem>
        <MusicListItem v-for="item in mvList" :key="item.id" :item="item" :wth="24" :topShow="false" :userShow="true"
          :mvType="true" v-show="!isTog"></MusicListItem>
      </div>
      <el-pagination layout="prev, pager, next" :page-size="limit" :total="totals" @current-change="handleCurrentChange"
        style="textAlign:center;margin:0 auto;marginBottom:20px"></el-pagination>
    </div>
  </div>
</template>
